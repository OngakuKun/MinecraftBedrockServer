...
Update_Service() {
  ...
  if [[ "$answer" != "${answer#[Yy]}" ]]; then
    ...
    echo "Current system time: $CurrentTime $TimeZone"
    ...
    if [[ "$answer" != "${answer#[Yy]}" ]]; then
      ...
      echo "Daily restart scheduled. To edit automatic restart type crontab -e"
    fi
  fi
}
...
Check_Dependencies() {
  ...
  if command -v apt-get &>/dev/null; then
    ...
    echo "[WARN] Your distro is not officially supported."
    echo "   You may need to install these yourself"
    echo "   curl, screen, unzip, gawk, openssl, xargs, pigz"
  fi
}
...
Update_Server() {
  ...
  echo "Latest Version: $DownloadFile"
  ...
  unzip -qo "downloads/$DownloadFile"
}
...
Check_Architecture() {
  ...
  if [[ "$CPUArch" == *"aarch"* ]]; then
    ...
    curl -H "Accept-Encoding: identity" -L -o depends.zip https://raw.githubusercontent.com/OngakuKun/MinecraftBedrockServer/master/depends.zip
    unzip depends.zip
    sudo mkdir /lib64
